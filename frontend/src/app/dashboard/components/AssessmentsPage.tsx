...
const deriveAssessments = (history: InterviewHistoryItem[]): AssessmentItem[] => {
  return history.map((item) => {
    ...
    return {
      ...
      skills: {
        top: strengths.length ? strengths.slice(0, 2) : [],
        weak: weaknesses.length ? weaknesses.slice(0, 2) : []
      },
      aiInsights: [
...
export default function AssessmentsPage({ interviewHistory, loading }: { interviewHistory: InterviewHistoryItem[]; loading: boolean }) {
  ...
  const [dbAssessments, setDbAssessments] = useState<AssessmentItem[]>([]);
  ...
  useEffect(() => {
    setAssessments([...dbAssessments, ...deriveAssessments(interviewHistory || [])]);
  }, [interviewHistory, dbAssessments]);
    ...
    <div className="cd-container">
      ...
      <div className="cd-stats-grid">
        ...
        <div className="cd-stat-card" style={{ display: 'flex', gap: '1rem', alignItems: 'center', padding: '1.25rem' }}>
          ...
          <div>
            <div className="cd-stat-value">{assessments.length ? Math.round(assessments.reduce((acc, curr) => acc + curr.completionRate, 0) / assessments.length) : 0}%</div>
        ...
        <div className="cd-stat-card" style={{ display: 'flex', gap: '1rem', alignItems: 'center', padding: '1.25rem' }}>
          ...
          <div>
            <div className="cd-stat-value">{assessments.length ? (assessments.reduce((acc, curr) => acc + curr.avgScore, 0) / assessments.length).toFixed(1) : 0}</div>
        ...
        <div className="cd-stat-card" style={{ display: 'flex', gap: '1rem', alignItems: 'center', padding: '1.25rem' }}>
          ...
          <div>
            <div className="cd-stat-value">{interviewHistory ? interviewHistory.reduce((acc, curr) => acc + (curr.qa_list ? curr.qa_list.filter(qa => qa.score <= 4).length : 0), 0) : 0}</div>

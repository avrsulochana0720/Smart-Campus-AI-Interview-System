...
export default function UsersRolesPage() {
  ...
  const getRoleBadge = (role: string) => {
    switch (role) {
      ...
      default: return <span style={{ padding: '0.2rem 0.5rem', borderRadius: '0.3rem', backgroundColor: 'rgba(148, 163, 184, 0.1)', color: '#64748B', fontSize: '0.75rem', fontWeight: 600, border: '1px solid rgba(148, 163, 184, 0.2)' }}>{role}</span>;
    }
  };
  ...
  return (
    <div style={{ paddingBottom: '2rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <div>
          <h2 style={{ fontSize: '1.5rem', fontWeight: 600, color: '#0F172A', margin: 0 }}>Users & Roles</h2>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: '1.5rem' }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr key={user.id} style={{ borderBottom: idx === users.length - 1 ? 'none' : '1px solid #334155', transition: 'background-color 0.2s' }}>
                    <td style={{ padding: '1rem' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                        ...
                        <div>
                          <div style={{ fontSize: '0.9rem', fontWeight: 500, color: '#0F172A' }}>{user.name}</div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          <div style={{ backgroundColor: '#FAF6EE', borderRadius: '0.75rem', border: '1px solid #334155', padding: '1.5rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem' }}>
              <Shield size={18} color="#E11D48" />
              <h3 style={{ fontSize: '1rem', fontWeight: 600, color: '#0F172A', margin: 0 }}>Permissions Overview</h3>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {[
                ...
                <div key={i} style={{ borderBottom: i === 2 ? 'none' : '1px solid #334155', paddingBottom: i === 2 ? 0 : '1rem' }}>
                  <div style={{ fontSize: '0.85rem', color: '#334155', marginBottom: '0.5rem', fontWeight: 500 }}>{perm.module}</div>
      ...
      {showInviteModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ backgroundColor: '#FAF6EE', padding: '2rem', borderRadius: '0.75rem', border: '1px solid #334155', width: '400px', maxWidth: '90%' }}>
            <h2 style={{ fontSize: '1.25rem', color: '#0F172A', margin: '0 0 1.5rem 0' }}>Invite New User</h2>
            ...
            <div style={{ display: 'flex', gap: '1rem', marginTop: '2rem' }}>
              <button onClick={() => setShowInviteModal(false)} style={{ flex: 1, padding: '0.75rem', backgroundColor: 'transparent', border: '1px solid #475569', color: '#64748B', borderRadius: '0.5rem', cursor: 'pointer' }}>Cancel</button>
              <button onClick={() => {
      ...
      {showPermissionsModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ backgroundColor: '#FAF6EE', padding: '2rem', borderRadius: '0.75rem', border: '1px solid #334155', width: '600px', maxWidth: '90%' }}>
            <h2 style={{ fontSize: '1.25rem', color: '#0F172A', margin: '0 0 1.5rem 0' }}>Manage Global Permissions</h2>
              ...
              <tbody>
                {['Interview Management', 'Candidate Data', 'System Settings', 'Billing & Plans'].map((mod, idx) => (
                  <tr key={idx} style={{ borderBottom: '1px solid #334155' }}>
                    <td style={{ padding: '1rem', textAlign: 'left', color: '#334155', fontSize: '0.85rem' }}>{mod}</td>
            ...
            <div style={{ display: 'flex', gap: '1rem' }}>
              <button onClick={() => setShowPermissionsModal(false)} style={{ flex: 1, padding: '0.75rem', backgroundColor: 'transparent', border: '1px solid #475569', color: '#64748B', borderRadius: '0.5rem', cursor: 'pointer' }}>Cancel</button>
              <button onClick={() => {

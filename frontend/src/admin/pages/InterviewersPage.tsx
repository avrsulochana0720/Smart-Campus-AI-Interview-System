  ...
  return (
    <div style={{ paddingBottom: '2rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <div>
          <h2 style={{ fontSize: '1.5rem', fontWeight: 600, color: '#0F172A', margin: 0 }}>Interviewers</h2>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '1.5rem' }}>
        {loading ? (
          ...
          <div key={interviewer.id} style={{ backgroundColor: '#FAF6EE', borderRadius: '0.75rem', border: '1px solid #334155', padding: '1.5rem', position: 'relative' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1rem' }}>
              <div style={{ width: '48px', height: '48px', borderRadius: '50%', backgroundColor: '#334155', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#64748B', fontSize: '1.2rem', fontWeight: 600 }}>
                {(interviewer.name || 'I').charAt(0).toUpperCase()}
              </div>
              <div>
                <h3 style={{ fontSize: '1rem', color: '#0F172A', fontWeight: 600, margin: '0 0 0.2rem 0' }}>{interviewer.name}</h3>
                <p style={{ fontSize: '0.75rem', color: '#E11D48', margin: 0 }}>{interviewer.department || 'General'}</p>
              </div>
            </div>
            
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginBottom: '1.5rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.8rem', color: '#64748B' }}><Mail size={14} /> {interviewer.email}</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.8rem', color: '#64748B' }}><UserCheck size={14} /> {interviewer.role}</div>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px solid #334155', paddingTop: '1rem' }}>
              ...
              <div style={{ textAlign: 'center', borderLeft: '1px solid #334155', paddingLeft: '1.5rem' }}>
                <div style={{ fontSize: '1.2rem', fontWeight: 600, color: '#0F172A', display: 'flex', alignItems: 'center', gap: '0.25rem', justifyContent: 'center' }}><Video size={16} color="#E11D48" /> {interviewer.status === 'Active' ? 'Yes' : 'No'}</div>
      ...
      {isAdding && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ backgroundColor: '#FAF6EE', padding: '2rem', borderRadius: '0.75rem', border: '1px solid #334155', width: '400px', maxWidth: '90%' }}>
            <h2 style={{ fontSize: '1.25rem', color: '#0F172A', margin: '0 0 1.5rem 0' }}>Add New Interviewer</h2>

    ...
    <div style={{ paddingBottom: '2rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1.5rem', backgroundColor: '#0D1322', padding: '1rem', borderRadius: '0.75rem', border: '1px solid #1E293B' }}>
        ...
        <div style={{ display: 'flex', gap: '0.75rem' }}>
          <div style={{ position: 'relative' }}>
            <button onClick={() => setTypeFilterOpen(!typeFilterOpen)} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', backgroundColor: '#111827', color: '#94A3B8', border: '1px solid #1E293B', borderRadius: '0.5rem', padding: '0.5rem 1rem', fontSize: '0.85rem', cursor: 'pointer' }}><Filter size={14}/> {typeFilter}</button>
            {typeFilterOpen && (
              <div style={{ position: 'absolute', top: '100%', left: '0', marginTop: '0.5rem', backgroundColor: '#1E293B', border: '1px solid #334155', borderRadius: '0.5rem', padding: '0.25rem', zIndex: 10, minWidth: '150px', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.5)' }}>
                {['All Types', 'Engineering', 'Marketing', 'Sales'].map(opt => (
                  <div key={opt} onClick={() => { setTypeFilter(opt); setTypeFilterOpen(false); showToast(`Filter changed to ${opt}`, 'success'); }} style={{ padding: '0.5rem', fontSize: '0.75rem', color: '#E2E8F0', cursor: 'pointer', textAlign: 'left', borderRadius: '0.25rem' }} onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#334155'} onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'}>
                    {opt}
                  </div>
                ))}
              </div>
          ...
          <div style={{ position: 'relative' }}>
            <button onClick={() => setDateFilterOpen(!dateFilterOpen)} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', backgroundColor: '#111827', color: '#94A3B8', border: '1px solid #1E293B', borderRadius: '0.5rem', padding: '0.5rem 1rem', fontSize: '0.85rem', cursor: 'pointer' }}><Calendar size={14}/> {dateFilter}</button>
            {dateFilterOpen && (
              <div style={{ position: 'absolute', top: '100%', right: '0', marginTop: '0.5rem', backgroundColor: '#1E293B', border: '1px solid #334155', borderRadius: '0.5rem', padding: '0.25rem', zIndex: 10, minWidth: '150px', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.5)' }}>
                {['All Time', 'Last 7 Days', 'Last 30 Days', 'This Year'].map(opt => (
                  <div key={opt} onClick={() => { setDateFilter(opt); setDateFilterOpen(false); showToast(`Date filter changed to ${opt}`, 'success'); }} style={{ padding: '0.5rem', fontSize: '0.75rem', color: '#E2E8F0', cursor: 'pointer', textAlign: 'left', borderRadius: '0.25rem' }} onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#334155'} onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'}>
                    {opt}
                  </div>
                ))}
              </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(350px, 1fr))', gap: '1.5rem' }}>
        {loading ? (
          ...
          <div key={idx} style={{ backgroundColor: '#0D1322', borderRadius: '0.75rem', border: '1px solid #1E293B', padding: '1.5rem', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', transition: 'background-color 0.2s', cursor: 'pointer' }} onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#111827'} onMouseLeave={(e) => e.currentTarget.style.backgroundColor = '#0D1322'}>
            <div style={{ display: 'flex', gap: '1rem' }}>
              ...
              <div>
                <h3 style={{ fontSize: '1rem', color: '#FFFFFF', fontWeight: 600, margin: '0 0 0.25rem 0', lineHeight: 1.3 }}>{report.candidate_name} Evaluation</h3>
      ...
      {isGenerating && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ backgroundColor: '#0D1322', padding: '2rem', borderRadius: '0.75rem', border: '1px solid #1E293B', width: '400px', maxWidth: '90%' }}>
            <h2 style={{ fontSize: '1.25rem', color: '#FFF', margin: '0 0 1.5rem 0' }}>Generate Custom Report</h2>
            
            <div style={{ marginBottom: '1rem' }}>
              <label style={{ display: 'block', color: '#94A3B8', fontSize: '0.8rem', marginBottom: '0.5rem' }}>Candidate Name</label>
              <input type="text" value={newReport.candidate_name} onChange={(e) => setNewReport({...newReport, candidate_name: e.target.value})} style={{ width: '100%', padding: '0.75rem', backgroundColor: '#111827', border: '1px solid #1E293B', borderRadius: '0.5rem', color: '#FFF', outline: 'none' }} placeholder="e.g. John Doe" />
            </div>

            <div style={{ marginBottom: '1rem' }}>
              <label style={{ display: 'block', color: '#94A3B8', fontSize: '0.8rem', marginBottom: '0.5rem' }}>Job Role</label>
              <input type="text" value={newReport.job_role} onChange={(e) => setNewReport({...newReport, job_role: e.target.value})} style={{ width: '100%', padding: '0.75rem', backgroundColor: '#111827', border: '1px solid #1E293B', borderRadius: '0.5rem', color: '#FFF', outline: 'none' }} placeholder="e.g. Software Engineer" />
            </div>

            <div style={{ marginBottom: '1rem' }}>
              <label style={{ display: 'block', color: '#94A3B8', fontSize: '0.8rem', marginBottom: '0.5rem' }}>Final Score</label>
              <input type="number" value={newReport.final_score} onChange={(e) => setNewReport({...newReport, final_score: parseInt(e.target.value) || 0})} style={{ width: '100%', padding: '0.75rem', backgroundColor: '#111827', border: '1px solid #1E293B', borderRadius: '0.5rem', color: '#FFF', outline: 'none' }} min="0" max="100" />
            </div>

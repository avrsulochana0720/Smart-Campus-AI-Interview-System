  ...
  return (
    <div style={{ paddingBottom: '2rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <div>
          <h2 style={{ fontSize: '1.5rem', fontWeight: 600, color: '#0F172A', margin: 0 }}>Integrations Hub</h2>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '1.5rem' }}>
        {loading ? (
          ...
          <div key={i} style={{ backgroundColor: '#FAF6EE', borderRadius: '1rem', border: '1px solid #334155', padding: '1.5rem', display: 'flex', flexDirection: 'column' }}>
            ...
            <h3 style={{ fontSize: '1.1rem', fontWeight: 600, color: '#0F172A', margin: '0 0 0.5rem 0' }}>{app.name}</h3>
            <p style={{ fontSize: '0.85rem', color: '#64748B', margin: '0 0 1.5rem 0', lineHeight: 1.5, flex: 1 }}>{app.desc}</p>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ backgroundColor: '#FAF6EE', padding: '2rem', borderRadius: '0.75rem', border: '1px solid #334155', width: '500px', maxWidth: '90%' }}>
            ...
            <div style={{ display: 'flex', gap: '1rem' }}>
              <button onClick={() => setConfigureApp(null)} style={{ flex: 1, padding: '0.75rem', backgroundColor: 'transparent', border: '1px solid #475569', color: '#64748B', borderRadius: '0.5rem', cursor: 'pointer' }}>Cancel</button>
              <button onClick={() => {
      ...
      {actionApp && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ backgroundColor: '#FAF6EE', padding: '2rem', borderRadius: '0.75rem', border: '1px solid #334155', width: '400px', maxWidth: '90%', textAlign: 'center' }}>
            ...
            <h2 style={{ fontSize: '1.25rem', color: '#0F172A', margin: '0 0 1rem 0' }}>
            ...
            <div style={{ display: 'flex', gap: '1rem' }}>
              <button onClick={() => setActionApp(null)} style={{ flex: 1, padding: '0.75rem', backgroundColor: 'transparent', border: '1px solid #475569', color: '#64748B', borderRadius: '0.5rem', cursor: 'pointer' }}>Cancel</button>
              <button onClick={() => handleToggleStatus(actionApp)} style={{ flex: 1, padding: '0.75rem', backgroundColor: actionApp.status === 'Connected' ? '#EF4444' : '#E11D48', border: 'none', color: '#0F172A', borderRadius: '0.5rem', cursor: 'pointer' }}>

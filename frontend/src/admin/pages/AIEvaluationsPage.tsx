  ...
  return (
    <div style={{ paddingBottom: '2rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <div>
          <h2 style={{ fontSize: '1.5rem', fontWeight: 600, color: '#FFFFFF', margin: 0 }}>AI Evaluations Logs</h2>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.5rem', marginBottom: '2rem' }}>
        ...
        <div style={{ backgroundColor: '#0D1322', border: '1px solid #1E293B', borderRadius: '0.75rem', padding: '1.5rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <div><div style={{ fontSize: '0.85rem', color: '#94A3B8' }}>Flagged Sessions</div><div style={{ fontSize: '1.5rem', fontWeight: 700, color: '#FFFFFF' }}>{logs.filter(l => l.flag).length}</div></div>
        </div>
        <div style={{ backgroundColor: '#0D1322', border: '1px solid #1E293B', borderRadius: '0.75rem', padding: '1.5rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <div><div style={{ fontSize: '0.85rem', color: '#94A3B8' }}>Auto-Graded</div><div style={{ fontSize: '1.5rem', fontWeight: 700, color: '#FFFFFF' }}>{logs.filter(l => !l.flag).length}</div></div>
        </div>
      </div>
              ...
              <tr key={idx} style={{ borderBottom: idx === logs.length - 1 ? 'none' : '1px solid #1E293B' }}>
                ...
                <td style={{ padding: '1rem', textAlign: 'right' }}>
                  {log.status === 'Review Required' ? (
                    ...
                    <button onClick={() => setSelectedLog(log)} style={{ backgroundColor: 'transparent', color: '#64748B', border: '1px solid #334155', borderRadius: '0.25rem', padding: '0.4rem 0.75rem', fontSize: '0.75rem', fontWeight: 500, cursor: 'pointer' }}>View Details</button>
                  )}
                </td>
              </tr>
            ))}
      ...
      {selectedLog && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ backgroundColor: '#0D1322', padding: '2rem', borderRadius: '0.75rem', border: '1px solid #1E293B', width: '500px', maxWidth: '90%' }}>
            ...
            <div style={{ marginBottom: '1.5rem', backgroundColor: '#111827', padding: '1rem', borderRadius: '0.5rem', border: '1px solid #1E293B' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div>
                  <div style={{ fontSize: '0.75rem', color: '#64748B', marginBottom: '0.25rem' }}>Candidate</div>
                  <div style={{ color: '#E2E8F0', fontSize: '0.9rem' }}>{selectedLog.candidate}</div>
                </div>
                <div>
                  <div style={{ fontSize: '0.75rem', color: '#64748B', marginBottom: '0.25rem' }}>Time Logged</div>
                  <div style={{ color: '#E2E8F0', fontSize: '0.9rem' }}>{selectedLog.time}</div>
                </div>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: '1rem', marginTop: '2rem' }}>
              <button onClick={() => setSelectedLog(null)} style={{ flex: 1, padding: '0.75rem', backgroundColor: 'transparent', border: '1px solid #334155', color: '#94A3B8', borderRadius: '0.5rem', cursor: 'pointer' }}>Close Details</button>
              {selectedLog.status === 'Review Required' && (
                <button onClick={() => {
                  ...
                  setSelectedLog(null);
                }} style={{ flex: 1, padding: '0.75rem', backgroundColor: '#22C55E', border: 'none', color: '#FFF', borderRadius: '0.5rem', cursor: 'pointer' }}>
                  Override & Approve
                </button>
              )}
            </div>
